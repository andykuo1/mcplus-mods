...
public class EventFillCauldronHandler
{
	...
	public void onPlayerInteract(PlayerInteractEvent parEvent)
	{
		if (parEvent.entity instanceof EntityPlayer && parEvent.action.equals(PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK))
		{
			...
			if (itemstack != null && itemstack.getItem() == MCP_WoodenBucket.woodenBucketWater)
			{
				...
				if (block instanceof BlockCauldron)
				{
					...
					if (metadata < 3)
					{
						if (!entityplayer.capabilities.isCreativeMode)
						{
							...
							entityplayer.inventory.setInventorySlotContents(entityplayer.inventory.currentItem, MCP_WoodenBucket.woodenBucketWater.getContainerItem(itemstack));
						}
