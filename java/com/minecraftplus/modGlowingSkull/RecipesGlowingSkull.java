...
public class RecipesGlowingSkull implements IRecipe
{
	private Block block;
	private Item skull;
	
	public RecipesGlowingSkull(Block par1Block, Item par2Item)
	{
		...
		for (int i = 0; i < par1InventoryCrafting.getSizeInventory(); ++i)
		{
			...
			if (itemstack2.getItem() == Items.skull)
			{
				...
				if (i + (par1InventoryCrafting.getSizeInventory() == 9 ? 3 : 2) < par1InventoryCrafting.getSizeInventory())
				{
					...
					if (itemstack2 != null && itemstack2.getItem() == Item.getItemFromBlock(this.block))
					{
						flag = true;
